...
def method_Monte_Carlo_1(alpha, init_val, file):
    n = init_val
    ...
    if n < 100000000:
        while True:
            ...
            if n >= quantity_of_steps(val_qu):
                print(f"\tmethod_1(Monte-Carlo):\n\t\tQ: {np.mean(val_qu)}", f"|| num_of_steps(length)={n}", file=file)
                break
            n += 1
    else:
        print("\tmethod_1(Monte-Carlo):\n\t\tlack of resources due to sample length!!", file=file)


def method_2(alpha, init_val, file):
    n = init_val
    while True:
        ksi = create_ksi(alpha, n)
...
def method_3(alpha, init_val, file):
    n = init_val
    while True:
        etha = create_etha(n)
...
def method_4(alpha, init_val, file):
    n = init_val
    while True:
...
if __name__ == '__main__':
    ...
    error = 0.01
    alpha = [1, 0.3, 0.1]
    test_list_init_1 = [78000, 4190000, 330000000]
    test_list_init_2 = [11000, 1035000, 82550000]
    test_list_init_3 = [42500, 281500, 325000]
    test_list_init_4 = [61000, 50, 5]

    if os.path.exists("Results\\task_2_result.txt"):
        os.remove("Results\\task_2_result.txt")
    for i, value in enumerate(alpha):
        with open("Results\\task_2_result.txt", "a") as f:
            print(f"alpha: {value}", file=f)
            method_Monte_Carlo_1(value, test_list_init_1[i], f)
            method_2(value, test_list_init_2[i], f)
            method_3(value, test_list_init_3[i], f)
            method_4(value, test_list_init_4[i], f)
            print(f"For alpha = {value} was calculated.")
